...
4. **Dev builds are expected.** A build from a commit after the last tag reports `X.Y.(Z+1).devN+g<hash>`. This is normal and must not be "fixed" by pinning a version.
5. **Clean release builds.** A release wheel is built from a checkout whose `HEAD` is the tag with a clean tree, so the version is exactly `X.Y.Z` with no `.devN`/`+dirty` suffix.
